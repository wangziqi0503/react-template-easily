...
export const getBasePath = (type) => {
    ...
    switch (type) {
        case 'local':
            ...
            break
        case 'carefree':
            url = '//page.jd.com/exploit/carefree-test/carH5/dist'
            break
        case 'cargw':
...
// iscrool 自定义参数列表
export const iscrollWrapperStyle = {
    height: '100%',
    overflow: 'hidden'
}
...
export const resizeForInput = function (scrollHeight) {
    const deferTime = 500
    setTimeout(() => {
...
// 写入localStorage
export const writeStorage = function (key, value) {
    localStorage.setItem(key, value)
}

// 读取localStorage
export const readStorage = function (key) {
 ...
 */
export const loadImages = function (imagesArrUrl, callback) {
...
export const formatOrder = (value) => {
 ...
 * @return {*} 修改后的值
 */
export const formatTextForTirm = (value) => {
...
export const formatExchangeCode = (value) => {
    if (value.length > 19) {
        value = value.substr(0, 19)
    }
    if (isOnlyContainLetter(value) && value.substr(value.length - 1) != ' ') {
        return formatCode(value)
    }
    return formatCode(value.substr(0, value.length - 1))
}

// 判断字符串中是否仅仅包含数字
export const isOnlyContainDigit = (value) => {
    // 去除空格
...
// 判断字符串是否仅仅包含大小写字母
export const isOnlyContainLetter = (value) => {
    // 去除空格
    const tmp = value.replace(/\s+/g, '')
...
// 计算地图上两个坐标点之间的距离
export const getDistance = (sSrcPointX, sSrcPointY, sDesPointX, sDesPointY) =>
...
// 初始化屏幕webview高度
export const webViewHeight = window.document.body.offsetHeight
...
export const parseUrlSringToObject = (str) => {
    const obj = {}
    if (!str) {
        return {}
    }
 ...
 */
export const parseObjectToUrlString = (obj) => {
    const arr = []
    for (const k in obj) {
...
export const isNull = (obj) => !obj && obj !== false && obj !== 0
 ...
 * @return index Int
 */
export const getObjectInArrayIndex = (obj, list) => {
...
export const setUserAddress = (response) => {
    ...
    if (response.code == 0 && response.data != null && response.data.defaultAddress) {
        ...
        // console.log(getCookie('person_provinceName'))
    } else {
...
export const fullImg = (src, size, host) => {
