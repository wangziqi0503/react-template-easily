...
class App extends React.Component {
  ...
  createTitle() {
    ...
    const item = <td key="listtitle" style={{ width: '2.5rem' }} />;
    columns.unshift(item);
    return (
      <table className="list-title-table">
        <thead>
          <tr>{columns}</tr>
        </thead>
      </table>
    ...
    return this.props.datas.map((data, index) => {
      ...
      if (data.describe) {
        descriptionComponent = (
          <span className="list-name" ref={`description${index}`}>
            <span
      ...
      const keyDatas = data.datas.map((keydata, keyindex) => (
      ...
      const item = (
        <td key={`sbutitle-${index}`} style={{ width: '2.5rem' }}>
  ...
  render() {
    const list = this.createLists();
    const title = this.createTitle();
    return (
      <div className="cc-tablelist">
        {title}
        <ul
          className={`cc-datalist-list ${this.transition}`}
          ref="ul"
          style={{
            height: 'auto',
