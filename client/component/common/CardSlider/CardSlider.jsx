...
class CardSlider extends Component {
  constructor(props) {
     ...
     */
    silderHandler(size) {
      const { autoSilderTimespan } = this.props;
      const me = this;

      if (me.refs.myscrollView) {
        const curPage = me.refs.myscrollView.getIScrollInstance().currentPage;
        const { pageX } = curPage;
        const { pageY } = curPage;
        let nextPage = pageX + 1;
    ...
    startAutoSilder() {
      ...
      } else if (this.refs.myscrollView) {
        const curPage = this.refs.myscrollView.getIScrollInstance().currentPage;
        const { pageX } = curPage;
        const nextPage = pageX + 1;
        if (pageX == 0) {
          this.refs.myscrollView.getIScrollInstance().goToPage(1, 0, 0);
        }
      }
    ...
    generateCardContent(children) {
      ...
      return _children.map((item, idx) => {
        ...
        return (
          <li
            className="cardSlider_item_container"
            key={idx}
    ...
    generateIndicators(children) {
      return children.map((item, idx) => {
        ...
        return (
          <li className={`indicator_item ${current}`} key={idx} />
        );
      });
    }
    ...
    render() {
      ...
      if (children.length <= 1) {
        content = (
          <div className="cardSlider_container" style={{ height: this.props.containerHeight }}>
        ...
        const scrollViewProps = {
          ...
          onScrollEnd: () => {
            if (this.refs.myscrollView) {
              // 圆点相关逻辑
              const curPage = this.refs.myscrollView.getIScrollInstance().currentPage.pageX;
              let curIndicator = 1;
              if (curPage == 2) {
                curIndicator = 1;
        ...
        content = (
          <div
            ref="cardSlider_container"
            className="cardSlider_container"
...
CardSlider.CardItem = (props) => <div className="card_item" />;
