...
class PlainList extends Component {
  constructor(props) {
    ...
    itemClick(val, data) {
      ...
      if (index > -1) {
        list = list.filter((item, index, array) => (item !== val));
      } else {
        list.push(val);
    ...
    checkAll(flag, data) {
      const checkedList = this.state.checkeds;
      if (flag) {
      ...
      for (let i = 0; i < datas.length; i++) {
        ...
        const item = (
          <ListItem
            ...
            id={i}
            onClickHandler={this.itemClick.bind(this, datas[i].value, datas)}
            hasCheckbox={this.props.hasCheckbox}
            hasKey={this.props.hasKey}
            station={datas[i].stationName}
            value={datas[i].value}
            keyValue={datas[i].key}
            key={i}
          />
        );
        items.push(item);
        ...
        <div id="plainList" style={{ width: '100%', height: this.props.containerHeight, backgroundColor: '#eaeaea' }}>
          ...
          <ScrollView>
            <button className={this.props.btnClass} onClick={this.checkAll.bind(this, this.state.isAllChecked, datas)}>
              <input type="checkbox" className="demon-checkbox" checked={this.state.isAllChecked} />
              全选
            </button>
    ...
    const checkboxContainer = (
      <span>
        <input
          ...
          className="demon-checkbox"
          value={this.props.value}
        />
        <span className={`check-icon ${checkedStyle}`} />
      </span>
    );
      ...
      <li>
        <label className="content-top" onClick={this.props.onClickHandler}>
          <div className="flex">
            <p>
              {' '}
              {this.props.hasKey ? (`${this.props.keyValue}:`) : '' }
              {this.props.station}
            </p>
          </div>
