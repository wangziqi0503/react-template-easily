...
class ProcessBar extends Component {
  constructor(props) {
    ...
    render() {
      const {
        currentValue, maxValue, width, height, className, bgcolor, processColor, showText,
      } = this.props;
      ...
      const processValue = `${(currentValue / maxValue) * (width.replace(/[rem]|[px]/g, ''))}rem`;

      let processWidth = processValue;

      // 如果小于4px,则看不出圆角
      const t_width = (currentValue / maxValue) * (width.replace(/[rem]|[px]/g, '')) * this.elFontSize;
