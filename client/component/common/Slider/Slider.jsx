...
class Slider extends Component {
  constructor(props) {
    ...
    touchMoveHandler(event) {
      ...
      // 移动的位置,相对于窗口
      const endPosition_x = event.touches[0].clientX;
      // pointer起始位置,相对于窗口
      const slider_startPosition_x = this.getSliderPositionX();
      // 移动距离 --实际距离
      const moveRealDistance = endPosition_x - slider_startPosition_x;
    ...
    touchEndHandler(event) {
      this.preventEvent(event);
      const moveRealDistance = event.changedTouches[0].clientX - this.getSliderPositionX();
      this.setDistance(moveRealDistance);
    }
    ...
    clickHandler(event) {
      ...
      const moveRealDistance = event.clientX - this.getSliderPositionX();
      this.setDistance(moveRealDistance);
    }
    ...
    render() {
      ...
      const realMoveDistance = `${this.state.moveDistance}px`;
        ...
        <div className={sliderClassName} onClick={this.clickHandler.bind(this)}>
          <div className="frc_slider" style={{ width: sliderRealWidth }} ref={(ref) => { this.slider = ref; }}>
            <div
              ...
              style={{ WebkitTransform: `translateX(${realMoveDistance})` }}
            />
            <div className="frc_tracker" style={{ width: realMoveDistance }} />
          </div>
        </div>
      );
    }
