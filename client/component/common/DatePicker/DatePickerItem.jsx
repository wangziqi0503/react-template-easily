...
import * as TimeUtil from './time.jsx';
import { shallowEqual } from './pureRender.jsx';
...
const getTimeType = (format) => {
  ...
  throw new Error('时间格式必须包含 Y, M, D, h, m ,s 或 T字母');
};
...
class DatePickerItem extends Component {
    ...
    _iniDates(date) {
      ...
      const dates = Array(...Array(DATE_LENGTH))
        .map((value, index) => TimeUtil[`next${typeName}`](date, index - MIDDLE_INDEX));
      this.setState({ dates });
    }
    ...
    handleStart(event) {
      this.touchY = (!isUndefined(event.targetTouches)
    ...
    handleMove(event) {
      const touchY = (!isUndefined(event.targetTouches)
                && !isUndefined(event.targetTouches[0]))
        ? event.targetTouches[0].pageY
        : event.pageY;
