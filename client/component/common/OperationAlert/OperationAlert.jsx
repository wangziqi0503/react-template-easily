...
class OperationAlert extends React.Component {
  ...
  render() {
    const { operationAlertShow } = this.props;
    const mask = (
      <div className="operation-maskName" />
    );

    const alert = (
      <div className="operation-alertName" style={{ width: this.props.imgWidth, height: this.props.imgHeight }}>
        <div className="operationAlert-close-banner" onClick={this.closeAlert.bind(this)} />
        <img
          className={`${'operation-alert-img' + ' '}${this.props.className}`}

          ref="imgDiv"
          src={this.props.imgSrc}
          onLoad={this.handleImgLoaded.bind(this)}
          onError={this.handleImgErrored.bind(this)}
          onClick={this.onClickHandler.bind(this)}
        />
      </div>
    );

    const contentAlert = this.state.isVisible ? (
      <div>
        {mask}
        {alert}
      </div>
