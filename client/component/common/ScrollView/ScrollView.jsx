  ...
  constructor(props) {
    ...
    scrollEndHandler() {
      ...
      if (this.props.isNeedPullLoading && this.props.onPullupFetch) {
        const scrollViewInstance = this.myScroll;
        // 滑动到底部的判断,触发上拉动作
    ...
    render() {
      const {
        scrollerClassName, wrapperClassName, isNeedPullLoading, pullLoadingStatus, pullDownStatus, isNeedPullDown,
      } = this.props;

      return (
        <div className={`${this.wrapClassForInstance} ${wrapperClassName} frc_scrollView_wrapper`}>
          <div
            className={`${scrollerClassName} frc_scrollView_scroller`}
                       ...
                       isNeedPullDown
                         ? (
                           <div className="pullDownLoading">
                             <span className={`icon ${this.pullDownIcon[pullDownStatus]}`} />
